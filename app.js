...
app.get("/", async (req, res) => {
  ...
  works.push(await Works.find({ type: "certificates" }));
  works.push(await Works.find({ type: "hobby" }));
  works.push(await Works.find({ type: "others" }));
...
app.get("/downloadCV", async (req, res) => {
  res.download(path.join(__dirname, "static/CV/Anish CV.pdf"));
});
// login and dashboard
app.get("/login", async (req, res) => {
  // bcrypt.hash("anish2001", saltRounds, function (err, hash) {
  //   console.log(hash);
  // });
  res.render("login");
});
...
app.get("/go", async (req, res) => {
  const doc = await PersonalDetails.create(req.body);
  res.send("Okay");
});
  ...
  try {
    if (req.cookies.user == process.env.EMAIL) {
      if (req.files) {
        ...
        let noErrors = true;
        file.mv(
          path.join(__dirname, "/static/CV/Anish CV.pdf"),
          function (err) {
            if (err) {
              noErrors = false;
              console.log(err);
